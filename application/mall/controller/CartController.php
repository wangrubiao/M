...
class CartController extends CommonController {
    ...
    public function cartGoods(){
        ...
        if(unserialize(cookie('cart'))){
            ...
            $where['mall_specfocus.id'] = array('in',i_array_column(unserialize(cookie('cart')),'sku_id'));
        }else{
            //$val['mall_specfocus.id'] = array('in',array(0),'sku_id');
            $where['mall_specfocus.id'] = array('in',array(0));
        }
        $where['mall_picture.sort'] = array('eq',1); //设置第一张主图为购物车显示
